...
class Stack(maxSize: Int = 1000):
  ...
  /** Pop the top frame from the stack
    */
  def pop(): Frame =
    if frames.isEmpty then throw new RuntimeException("Cannot pop from empty thread stack")
    frames.remove(frames.size - 1)

  /** Peek at the top frame without removing it
    */
  def peek(): Frame =
    if frames.isEmpty then throw new RuntimeException("Cannot peek empty thread stack")
    frames.last
