...
import tinyjvm.memory.{OperandStack, LocalVariables}
import tinyjvm.classloader.{ConstantPoolEntry, Utf8Entry, ClassEntry, NameAndTypeEntry}

/**
 * A frame represents a method invocation on the JVM stack
 * Contains: operand stack, local variables, program counter, and constant pool reference
 */
class Frame(
  val maxStack: Int,
  val maxLocals: Int,
  ...
  def nextByte(): Byte =
    if pc >= code.length then
      throw new RuntimeException(s"Program counter out of bounds: pc=$pc, code.length=${code.length}")
    val byte = code(pc)
    pc += 1
    byte
